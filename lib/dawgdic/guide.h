...
class Guide {
 public:
  ...
  bool Write(IOFunction write, void *stream) const {
    ...
    if (!write(stream, const_cast<GuideUnit*>(units_), sizeof(GuideUnit) * size_)) {
      return false;
    }
  ...
  const void *Map(const void *address) {
    ...
    return reinterpret_cast<const uint8_t*>(address) + size_ * sizeof(GuideUnit) + sizeof(BaseType);
  }
